...
int RecursiveParser::ParseNamedRecord(const std::string& name) {
...
int RecursiveParser::ParseEnum(const std::string& name) {
  ...
  for (const auto& e : decl->enumerators()) {
    fields_layout.push_back({ decl->getIntegerType().getAsString(),
          e->getNameAsString(), TYPECLASS::TC_FUNDAMENTAL,
          std::stoi(e->getInitVal().toString(10)), { 1 } });
  }

  RecordLayout record{ rname, TYPECLASS::TC_ENUM, std::move(fields_layout) };
...
int RecursiveParser::ParseRecordLayout(const std::string& name,
  ...
  for (const auto* d : decl->fields()) {
    ...
    if (record_layout_.find(tstr) == record_layout_.end()) {
      if (field.tycls == TYPECLASS::TC_ENUM) {
        if (0 != ParseEnum(tstr)) {
          return -1;
        }
      } else if (field.tycls != TYPECLASS::TC_FUNDAMENTAL) {
        if (0 != ParseNamedRecord(tstr)) {
          return -1;
        }
      }
    }
  ...
  RecordLayout record{ name, TypeCheck(decl->getTypeForDecl()),
                       std::move(fields_layout) };
  record_layout_[record.name] = record;
